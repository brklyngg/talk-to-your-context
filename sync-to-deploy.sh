...
# Drop the obsolete idle-watchdog var if present (replaced by ASK_AGENT_IDLE_TIMEOUT_SEC).
if grep -q '^ASK_AGENT_IDLE_TIMEOUT=' "$env_file"; then
  sed -i.bak '/^ASK_AGENT_IDLE_TIMEOUT=/d' "$env_file"
  rm -f "$env_file.bak"
  echo "  - ASK_AGENT_IDLE_TIMEOUT (obsolete; replaced by ASK_AGENT_IDLE_TIMEOUT_SEC)"
fi

# Bump pinned ASK_AGENT_TIMEOUT_SEC=90 (pre-2026-05-09 default) to 600 — the 90s
# wall-clock cap was empirically below median Hermes action-class latency.
# Idempotent.
if grep -q '^ASK_AGENT_TIMEOUT_SEC=90$' "$env_file"; then
  sed -i.bak 's/^ASK_AGENT_TIMEOUT_SEC=90$/ASK_AGENT_TIMEOUT_SEC=600/' "$env_file"
  rm -f "$env_file.bak"
  echo "  ~ ASK_AGENT_TIMEOUT_SEC 90 → 600 (idle watchdog now primary; cap is runaway guard)"
fi
